...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    pagi_http::tracing::init("pagi-hive-sync-plugin");
    ...
    let plugin_url = std::env::var("PLUGIN_URL").unwrap_or_else(|_| "http://127.0.0.1:9050".to_string());

    let git = GitConfig {
        repo_url: std::env::var("HIVE_REPO_URL").unwrap_or_default(),
        local_path: std::env::var("HIVE_LOCAL_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from("/data/hive-repo")),
        base_branch: std::env::var("HIVE_BASE_BRANCH").unwrap_or_else(|_| "main".to_string()),
    ...
    let app = Router::new()
        ...
        .route("/hive_push", post(hive_push))
        .route("/hive_pull", post(hive_pull))
        .with_state(state)
        .layer(tower_http::trace::TraceLayer::new_for_http());

    let addr: SocketAddr = pagi_http::config::bind_addr(([0, 0, 0, 0], 9050).into());
    info!(%addr, "pagi-hive-sync-plugin listening");
...
async fn register_tools_with_gateway(state: &AppState) -> Result<(), String> {
    ...
    let tools = vec![
        GatewayToolSchema {
            name: "hive_push".to_string(),
            description: "Push a refinement artifact into the Hive knowledge repo (e.g., GitHub)".to_string(),
            plugin_url: state.plugin_url.clone(),
            endpoint: "/hive_push".to_string(),
            parameters: json!({
                "type": "object",
        ...
        GatewayToolSchema {
            name: "hive_pull".to_string(),
            description: "Pull the latest playbook from the Hive knowledge repo".to_string(),
            plugin_url: state.plugin_url.clone(),
            endpoint: "/hive_pull".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "twin_id": {"type": "string"}
    ...
    info!("registered hive sync tools with ExternalGateway");
    Ok(())
}
...
struct HivePullRequest {
    #[allow(dead_code)]
    pub twin_id: Option<TwinId>,
}

async fn hive_pull(State(state): State<AppState>, Json(_req): Json<HivePullRequest>) -> impl IntoResponse {
    let cfg = state.git.clone();
...
fn open_or_clone(cfg: &GitConfig) -> Result<Repository, String> {
    ...
    if cfg.repo_url.trim().is_empty() {
        return Err("HIVE_REPO_URL is required (no existing repo at HIVE_LOCAL_PATH)".to_string());
    }
...
fn git_push_artifact(cfg: &GitConfig, artifact: &RefinementArtifact) -> Result<String, String> {
    ...
    let artifacts_dir = cfg.local_path.join("hive_artifacts");
    std::fs::create_dir_all(&artifacts_dir).map_err(|e| e.to_string())?;

    let artifact_id = Uuid::new_v4();
    let rel_path = PathBuf::from(format!("hive_artifacts/{artifact_id}.toml"));
